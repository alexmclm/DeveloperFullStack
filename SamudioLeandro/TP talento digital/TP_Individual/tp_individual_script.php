...
$conexion = mysqli_connect($aux,"root","") or die("murio<br><br>");
...
$sql = "CREATE DATABASE $nombreBaseDatos";
  ...
  echo "La base de datos ya ha sido creada.<br><br>";
  }
  else{
...
// importante poner comillas en las variables de sql -> `Var` Tipo, CaracteristicaDeLaTabla
...
//------------ creo tabla tarea (ID, usuario_id, creacion, descripcion, fin)----------------------
  ...
  if ($conexion->query($sql) === TRUE) {
  echo "La tabla TAREA ha sido creada satisfactoriamente.<br><br>";
  } else {
  echo "Hubo un error al crear la tabla usuarios.<br><br>"/* . $conn->error*/;
  echo mysql_errno($conexion) . ": " . mysql_error($conexion). "\n";
}
